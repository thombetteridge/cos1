...
{
  Vector2 const pos = GetMousePosition();
...
{
  int const button  = (int)luaL_checkinteger(L, 1); // 0 = left, 1 = right, 2 = middle
...
int l_gui_slider(lua_State *L)
{
  ...
  f32 const minValue          = (f32)luaL_checknumber(L, 6);
  f32 const maxValue          = (f32)luaL_checknumber(L, 7);
...
int l_draw_text(lua_State *L)
{
  ...
  f32 const x        = (f32)luaL_checknumber(L, 2);
  f32 const y        = (f32)luaL_checknumber(L, 3);
  f32 const fontSize = (f32)luaL_checknumber(L, 4);
